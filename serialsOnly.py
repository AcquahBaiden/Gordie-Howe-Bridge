...
print("Baiden passed imports")
usb_paths = getdevices.serial_ports('/dev/sd*')

if len(usb_paths) == 0:
    local_dir = "/home/pi/Gordie-Howe-Bridge/data/serialsOnly/"
...
def main_thread():
    ...
    print("Baiden Main thread 1")
    
    if NUM_MIN_RUN is None:
        print ("Baiden Checking conditions")
        ...
        print("Baiden about to close connections")
        close_connections()

    print("About to get ad and aero status")
    ...
    if not STATUS_FLAG_DICT['sd']:
        print("Inside status flag")
        d = datetime.now()
        str_d = d.strftime('%Y-%m-%d,%H:%M:%S:%f')
        if SD_NUM_OF_READS != 0:
            print("Baiden number of reads not equal to zero")
            file_sd.write("%s,%.3f,%.3f\r\n" % (str_d,SD_SUM/SD_NUM_OF_READS,SD_MAX))
        SD_SUM = 0
        SD_NUM_OF_READS = 0
        SD_MAX = 0

    print("Baiden about to enter write zone")
...
def main():
    ...
    if current_retry_number != MAX_NUM_RETRIES:
        ...
        print("about to call sd_thread")
        sd_thread()
        print("about to call main_thread")
        main_thread()
